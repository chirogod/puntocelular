<?php
	...
	if($datos_orden->rowCount()==1){
		...
		if($datos_orden['id_cliente']==1){
			...
			$pdf->SetFont('Arial','B',10);
			$pdf->Text(12, $yNewRect +10, 'Nombre: ');
			$pdf->SetFont('Arial','',10);
			$pdf->Text(28, $yNewRect +10, $datos_orden['cliente_nombre_completo']);
	
			$pdf->SetFont('Arial','B',10);
			$pdf->Text(12, $yNewRect +15, 'Direccion: ');
			$pdf->SetFont('Arial','',10);
			$pdf->Text(31, $yNewRect +15, $datos_orden['cliente_domicilio'].", ".$datos_orden['cliente_localidad'].", ".$datos_orden['cliente_provincia'].", ".$datos_orden['cliente_pais']);
	
			$pdf->SetFont('Arial','B',10);
			$pdf->Text($pageWidth/2+30, $yNewRect +10, 'Telefono: ');
			$pdf->SetFont('Arial','',10);
			$pdf->Text($pageWidth/2 + 48, $yNewRect +10, $datos_orden['cliente_telefono_1']." / ". $datos_orden['cliente_telefono_2']);
	
			$pdf->SetFont('Arial','B',10);
			$pdf->Text($pageWidth/2+30 , $yNewRect +15, 'Email: ');
			$pdf->SetFont('Arial','',10);
			$pdf->Text($pageWidth/2 + 42, $yNewRect +15, $datos_orden['cliente_email']);
		}

		$pdf->Ln(8);

		// RECTANGULO DETALLE DE ARTICULOS
		$pdf->Rect(10, $yNewRect +16, 16, 5); 
		$pdf->SetFont('Arial','b',10);
		$pdf->SetTextColor(0,0,0);
		$pdf->Text(12, $yNewRect+20, "Equipo");
		$pdf->Line(10, $yNewRect+21, $pageWidth-10, $yNewRect+21);
		...
		// ARRANCA COMPROBANTE PARA CLIENTE
		
		$yNewRect = $pdf->GetY();

		$pdf->Line(0, $yNewRect+2, $pageWidth, $yNewRect+2);


		// RECTANGULO PRINCIPAL
		$pdf->Rect(10, $yNewRect+5, $pageWidth - 20, 42);

		// TEXTO DEL LADO IZQUIERDO DEL RECTANGULO PRINCIPAL
		$pdf->SetFont('Arial','',10);
		$pdf->Ln(9);
		$pdf->Image('../views/img/logo.png', 16, $yNewRect+6, 48);
		$pdf->SetFont('Arial','B',10);
		$pdf->Ln(15);
		$pdf->Cell(150,9,"Reparaciones - Ventas - Accesorios" ,0,0,'L');
		$pdf->Ln(5);
		...
		// TEXTO DEL LADO DERECHO DEL RECTANGULO PRINCIPAL
		$pdf->SetFont('Arial','B',12);
		$pdf->Text($pageWidth / 2 + 35, $yNewRect+10, 'Entrada: ');
		$pdf->SetFont('Arial','',12);
		$pdf->Text($pageWidth / 2 + 52, $yNewRect+10,date("d/m/Y", strtotime($datos_orden['orden_fecha']))." - ". $datos_orden['orden_hora']  );

        $pdf->SetFont('Arial','B',12);
		$pdf->Text($pageWidth / 2 + 40, $yNewRect+20, 'Orden de reparacion');
		$pdf->SetFont('Arial','',18);
		$pdf->Text($pageWidth / 2 + 59, $yNewRect+28, $datos_orden['id_orden']);
        $pdf->SetFont('Arial','b',12);
		$pdf->Text($pageWidth / 2 + 40, $yNewRect+35, 'Estado:');
		$pdf->SetFont('Arial','',12);
		$pdf->Text($pageWidth / 2 + 56, $yNewRect+35, $datos_orden['orden_estado']);
		$pdf->SetFont('Arial','b',12);
		$pdf->Text($pageWidth / 2 + 25, $yNewRect+40, 'Tipo:');
		$pdf->SetFont('Arial','',12);
		$pdf->Text($pageWidth / 2 + 36, $yNewRect+40, $datos_orden['orden_tipo']);
		$pdf->Text($pageWidth / 2 + 63, $yNewRect+40, "para: ".$datos_orden['orden_fecha_prometida']);
		...
		// RECTANGULO DETALLE DE ARTICULOS
		$pdf->Rect(10, $yNewRect +18, 16, 5); 
		$pdf->SetFont('Arial','b',10);
		$pdf->SetTextColor(0,0,0);
		$pdf->Text(12, $yNewRect+22, "Equipo");
		$pdf->Line(10, $yNewRect+23, $pageWidth-10, $yNewRect+23);
        ...
        $pdf->Text(11, $yNewRect+32, "Falla declarada:");
        $pdf->SetFont('Arial','',7);
        ...
        $yNewRect = $yNewRect+16;
        $pdf->SetFont('Arial','b',10);
        $pdf->Text(11, $yNewRect+27, "Accesorios incluidos:");
        $pdf->SetFont('Arial','',7);
        $pdf->SetXY(48, $yNewRect+25);
		$pdf->MultiCell(160, 2.5, $datos_orden['orden_accesorios']);

        $pdf->Ln(2);
        $pdf->SetFont('Arial','b',10);
        $pdf->Text(11, $yNewRect+31, "Detalles fiscos:");
        $pdf->SetFont('Arial','',7);
		$pdf->SetXY(38, $yNewRect+29);
		$pdf->MultiCell(150, 2.5, $datos_orden['orden_equipo_detalles_fisicos']);

		$pdf->Ln(2);
        $pdf->SetFont('Arial','b',10);
        $pdf->Text(11, $yNewRect+35, "Observaciones:");
        $pdf->SetFont('Arial','',7);
		$pdf->SetXY(38, $yNewRect+33);
		$pdf->MultiCell(150, 2.5, $datos_orden['orden_observaciones']);

        $pdf->Line(10, $yNewRect+39, $pageWidth-10, $yNewRect+39);
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY(8, $yNewRect+40);
        $pdf->MultiCell($pageWidth-15, 2.5, "IMPORTANTE: Transcurridos los 15 dias a partir de la REPARACION y NOTIFICACION, la empresa cobrara al propietario del dispositivo un recargo sobre el presupuesto, en concepto de interes punitorio y/o moratorio. La tasa del mismo sera de un 10% quincenal. Asimismo, cumplidos los 90 dias posteriores a la REPARACION y NOTIFICACION del dipositivo, de no ser retirado, se considerara abandonado, facultando a PUNTO CELULAR a disponer del bien. En caso de no aceptar el presupuesto, luego de ingresado el dispositivo, se debera abonar $5.000 de servicio de presupuestao SIN EXCEPCION.", 0, 'L');
